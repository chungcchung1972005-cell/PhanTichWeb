...
- System prompt (trong `server/server.js`): giới hạn phạm vi tư vấn, không bịa số liệu cọc/chính sách, không tự chốt lịch trong chat, chỉ dẫn trang khi khách bấm nút gợi ý. Server này không dùng làm tiền đề cho backend CRM/booking sau này; stack CRM/booking vẫn chưa chốt.
